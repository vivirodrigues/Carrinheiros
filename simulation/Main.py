...
from geography import GeoTiff, OpenSteetMap
...
def geotiff_transformation(name_file):
    gdal_command = "gdal_translate -of GTiff -ot Int16 -co TFW=YES " + name_file + " " + name_file
...
def create_route(stop_points, material_weights):
    ...
    OpenSteetMap.file_osm(Constants.MAPS_DIRECTORY + Constants.FILE_NAME_OSM, stop_points)
    ...
    G, nodes_coordinates, nodes_mass_increment = Carrinheiro.graph_scenario(stop_points, geotiff_name, material_weights)
...
def main():
    ...
    paths, geotiff_name = create_route(stop_points, material_weights)

    geotiff_name = Constants.MAPS_DIRECTORY + geotiff_name + '.tif'

    geotiff_transformation(geotiff_name)

    netconvert_geotiff(Constants.MAPS_DIRECTORY + Constants.FILE_NAME_OSM, geotiff_name, name_file_net)

    sumo_route = []
    for i in paths:
        sumo_route.append(Map_osm.nodes_to_edges(i, name_file_net))
